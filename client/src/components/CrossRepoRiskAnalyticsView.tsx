...
import { CrossRepoRiskAnalytics as CrossRepoRiskAnalyticsComponent } from './CrossRepoRiskAnalytics';
import {
  getCrossRepoRiskAnalytics,
  type CrossRepoRiskAnalytics as CrossRepoRiskAnalyticsType,
} from '../api';
  ...
  const fetchAnalytics = useCallback(
    async (refresh: boolean = false) => {
      ...
      const message = refresh
        ? 'Recalculating cross-repo risk analytics across all repositories... This may take a while.'
        : 'Calculating cross-repo risk analytics across all repositories... This may take a while.';
      const loadingId = showNotification('loading', message, 0);
      loadingNotificationIdRef.current = loadingId;

      try {
        const data = await getCrossRepoRiskAnalytics(projectId, refresh);
        setAnalytics(data);
        ...
        const successMessage = refresh
          ? 'Cross-repo risk analytics recalculated successfully!'
          : 'Cross-repo risk analytics calculated successfully!';
        showNotification('success', successMessage, 3000);
      } catch (err) {
        const errorMessage = 'Failed to load cross-repo risk analytics';
  ...
  return (
    <>
      ...
      {loading && !analytics ? (
        ...
        <CrossRepoRiskAnalyticsComponent analytics={analytics} loading={loading} />
      ) : (
        <div className="text-center py-12 text-gray-500">
          No project selected. Select a project to view cross-repository risk analytics.
        </div>
      )}
    </>
  );
