...
export function CrossRepoRepositoryEvolutionView() {
  const params = useParams({ strict: false }) as { projectId?: string };
  const projectId = params?.projectId;
  const { projects } = useApp();
  const [evolution, setEvolution] = useState<CrossRepoRepositoryEvolutionType | null>(null);
    ...
    async (refresh: boolean = false) => {
      ...
      try {
        ...
        const errorMessage =
          ...
          'Failed to load cross-repo repository evolution metrics';
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            {projectId
              ? `Analyzing repository evolution and synchronization across all repositories in this project`
              : 'Select a project to view cross-repo repository evolution'}
          </p>
        </div>
        {projectId && <RecalculateButton loading={loading} onClick={() => fetchEvolution(true)} />}
      </div>
